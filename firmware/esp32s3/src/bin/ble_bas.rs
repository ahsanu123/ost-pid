    ...
    C: Controller,
{
    ...
    let _ = join(ble_task(runner), async {
        loop {
            match advertise("ah battery service", &mut peripheral, &server).await {
                Ok(conn) => {
...
async fn main(_s: Spawner) {
    ...
    let connector = BleConnector::new(&controller, bluetooth, Config::default()).unwrap();
